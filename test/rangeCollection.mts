...
test('rangeCollection', { concurrency: true }, t => {
    t.test('big add remove test', () => {
    ...
    t.test('enumerate tests', () => {
        ...
        let i = 0
        for (const x of [...enumerate(range)]) {
            assert.equal(x, numbers[i])
            i += 1
        }

        assert.equal(i, numbers.length)

        const notNumbers = numbersBase.filter(x => !added[x])

        i = 0
        for (const x of enumerateMissing(range, 0, numbersBase[numbersBase.length - 1])) {
            assert.equal(x, notNumbers[i], `Index: ${i}, range: ${x}, actual: ${notNumbers[i]}`)
            i += 1
        }
    });
});